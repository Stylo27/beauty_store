class ProductsSortQuery < ProductsFinderQuery
  def call(query_params)
    sorted_products = sort_by_name(products, query_params[:sort][:order_by_name])
  ...
  def sort_by_name(products, order_by_name)
    order_by_name ? products.order(name: order_by_name) : products
  end

  def sort_by_category(products, order_by_category)
    order_by_category ? products.order(category: order_by_category) : products
  end

  def sort_by_price(products, order_by_price, under_sale)
